...
class VolunteerAdmin(admin.ModelAdmin):
    list_display = ('x_handle', 'researcher', 'pipeline_status',
...
class BFISurveyAdmin(admin.ModelAdmin):
    list_display = ('volunteer', 'openness_display', 'conscientiousness_display',
                    'extraversion', 'agreeableness_display', 'neuroticism_display', 'completed_at')
    ...
    @admin.display(description='O')
    def openness_display(self, obj):
        formatted_value = f"{obj.openness:.2f}"
        return format_html('<span style="color: #1E90FF;">{}</span>', formatted_value)

    @admin.display(description='C')
    def conscientiousness_display(self, obj):
        formatted_value = f"{obj.conscientiousness:.2f}"
        return format_html('<span style="color: #32CD32;">{}</span>', formatted_value)

    @admin.display(description='E')
    def extraversion_display(self, obj):
        formatted_value = f"{obj.extraversion:.2f}"
        return format_html('<span style="color: #FFA500;">{}</span>', formatted_value)

    @admin.display(description='A')
    def agreeableness_display(self, obj):
        formatted_value = f"{obj.agreeableness:.2f}"
        return format_html('<span style="color: #9370DB;">{}</span>', formatted_value)

    @admin.display(description='N')
    def neuroticism_display(self, obj):
        formatted_value = f"{obj.neuroticism:.2f}"
        return format_html('<span style="color: #FF6347;">{}</span>', formatted_value)


@admin.register(POST)
class PostAdmin(admin.ModelAdmin):
    list_display = ('x_post_id', 'volunteer', 'selected_by_qlearning',
                    'engagement_score', 'created_at_original')
    list_filter = ('selected_by_qlearning',
                   'embedding_processed', 'created_at_original')
...
class SyntheticDataAdmin(admin.ModelAdmin):
    list_display = ('volunteer', 'original_post',
                    'generation_confidence', 'used_in_training', 'created_at')
...
class LassoModelAdmin(admin.ModelAdmin):
    list_display = ('volunteer', 'trait', 'is_active',
                    'train_mae', 'validation_mae', 'trained_at')
...
class PsychometricProfileAdmin(admin.ModelAdmin):
    list_display = ('volunteer', 'overall_mae',
                    'prediction_confidence', 'posts_analyzed', 'updated_at')
