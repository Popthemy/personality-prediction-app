...
REVERSE_ITEMS = {2, 6, 8, 9, 12, 18, 21, 23, 24,
                 27, 28, 29, 30, 33, 34, 35, 37, 38, 39, 41, 43}
        ...
        for item_str, score in responses.items():
            try:
                ...
                if not 1 <= item_num <= 44:
                    errors.append(
                        f"Item number {item_num} out of range (1-44)")
            except (ValueError, TypeError):
            ...
            try:
                ...
                if not 1 <= score_int <= 5:
                    errors.append(
                        f"Item {item_str}: score {score} out of range (1-5)")
            except (ValueError, TypeError):
...
def score_bfi_survey(responses: Dict[str, int]) -> Dict[str, float]:
    ...
    res = BFIScorer.calculate_all_traits(responses)
    print(f"Calculated BFI trait scores: {res}")
